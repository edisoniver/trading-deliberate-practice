...
//     Papa.parse('/ES_full_1min_continuous_UNadjusted.txt', {
//       download: true,
//       step: function (row, parser) {
//         console.log('Row data:', row.data);
//         if (row.errors.length === 0) {
//           const [dateTime, open, high, low, close, volume] = row.data; // Modify this line
...
const getData = async (date) => {
  const response = await fetch(`/daily_data/${date}.json`);
  if (!response.ok) {
